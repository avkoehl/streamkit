...
def upstream_length(G):
    ...
    for node in nx.topological_sort(G):
        ...
        if len(upstream) == 0:
            ...
            u, v, data = upstream[0]
            length = data.get("upstream_length") + data.get("geometry").length
        else:
            max_upstream_length = 0.0
            for u, v, data in upstream:
                upstream_length = (
                    data.get("upstream_length") + data.get("geometry").length
                )
        ...
        for _, v, out_data in G.out_edges(node, data=True):
            out_data["upstream_length"] = length
    return G
